...
const getBookingSummary = (refId, { title, isVoucher }) => {
  return TravelDashboardAPI.post(
    BOOKING_BASE + `/document/reservation/get-summary/${refId}`,
    { title, isVoucher },
    {
      headers: {
...
const getHotels = (request) => {
  return TravelDashboardAPI.post(BOOKING_BASE + `/get-hotel`, request)
    .then((res) => {
      return res.data.data;
    })
    .catch((error) => {
...
const getHotelRooms = (request) => {
    ...
    if (rooms?.[0]?.rates) {
      return rooms?.map((el) => ({
        ...
        bookingKey: JSON.stringify({
          bookingKey: el?.rates?.[0]?.bookingKey,
          identifier: uuidv4(),
        }),
        children: undefined,
        childs: el?.children,
  ...
  return TravelDashboardAPI.post(BOOKING_BASE + `/get-hotel-rooms`, request)
    .then((res) => {
      return {
        ...res.data.data,
...
const updateBookingStatus = (request) => {
  return TravelDashboardAPI.put(BOOKING_BASE + `/update-booking-status/${request.refId}`, {
...
const editBookingNote = (request) => {
  return TravelDashboardAPI.put(
    BOOKING_BASE + `/update-booking-notes/${request.bookingId}`,
    request.note,
  )
    .then((res) => {
      return res;
...
const getPayments = (refId, { page, size }) => {
  return TravelDashboardAPI.get(BOOKING_BASE + `/payment/list-payments/${refId}`, {
    params: { page, size },
  })
    .then((res) => {
      return res.data.data;
...
const editPayment = (payment) => {
  return TravelDashboardAPI.put(BOOKING_BASE + `/payment/edit-payment/${payment.id}`, {
    ...payment,
  })
    .then((res) => {
      return res.data.data;
...
const deletePayment = ({ id, refId }) => {
  return TravelDashboardAPI.delete(BOOKING_BASE + `/payment/delete-payment/${id}`, {
    data: { refId },
  })
    .then((res) => {
      return res.data.data;
...
// GET BOOK CANCEL POLICIES
const getBookCancelPolicies = (bookingId) => {
  return TravelDashboardAPI.get(BOOKING_BASE + `/get-book-cancel-policies/${bookingId}`)
    .then((res) => {
      return res.data.data;
    })
    .catch((error) => {
...
//  CANCEL ACCOMODATION BOOK
const cancelAccommodationBook = (payload) => {
  return TravelDashboardAPI.post(BOOKING_BASE + `/cancel-book`, payload)
    .then((res) => {
      return res.data.data;
    })
    .catch((error) => {
...
const BookingService = {
  ...
  editPassenger,
  // CANCEL
  getBookCancelPolicies,
  cancelAccommodationBook,
};
