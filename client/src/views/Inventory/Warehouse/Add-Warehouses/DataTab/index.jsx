...
export default function DataTab({
  ...
  const onFinish = async (values) => {
    ...
    console.log(values.warehousesImage, "values.warehousesImage");

    values.warehousesImage && formData.append("image", values.warehousesImage[0]?.originFileObj);
    ...
    WarehousesService.addWarehouse(formData).then(
      (data) => {
        setIsLoading(false);
        reset();
        onClose();
        setWarehousesdata((prev) => [...prev, data.data.data]);
    ...
    <>
      <Form form={form} onFinish={onFinish} layout="vertical" name="data">
        <section className="general-form">
          ...
          <Form.Item name="warehousesStatus" label="Status">
