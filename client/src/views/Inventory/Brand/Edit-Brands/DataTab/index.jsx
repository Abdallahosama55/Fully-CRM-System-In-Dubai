...
export default function DataTab({ resetTabs, setRefresh, onClose, brandsdata, data }) {
  const { openNotificationWithIcon } = useNotification();
  const [form] = useForm();
  const [previewPic, setPreviewPic] = useState(brandsdata.image ? brandsdata.image : "");
  const [previewPicData, setPreviewPicData] = useState("");

  const [isLoading, setIsLoading] = useState(false);

  const { dataRef, getCurrentbrands, resetForms } = useBransContext();
  ...
  const onFinish = async (values) => {
    ...
    BrandsService.editBrand(brandsdata.id, formData).then(
      () => {
        setIsLoading(false);
        reset();
        onClose();
    ...
    <>
      <Form form={form} onFinish={onFinish} layout="vertical" name="data">
        <section className="general-form">
          ...
          <Form.Item initialValue={brandsdata.showStatus} name="BrandsStatus" label="Status">
          ...
          <Form.Item initialValue={brandsdata.sortOrder} name="sortOrder" label="Sort Order">
