...
import { CalenderBtnSVG, DeleteSVG } from "assets/jsx-svg";

export default function PriceSection({ name, priceCurrency }) {
  const [specialPricePeriodOpen, setSpecialPricePeriodOpen] = useState(false);
  const [quantityPricePeriodOpen, setQuantityPricePeriodOpen] = useState(false);
          ...
          {(fields1, { add, remove }) => (
            <>
              ...
              {fields1.map(({ key, name }) => (
                <Row gutter={[10, 10]} className="mb-1" key={key}>
                  <Col span={24}>
                    <Form.Item noStyle>
                      <div className="mb-1"> Special Discount Period</div>
                      <Row wrap={false}>
          ...
          {(fields2, { add, remove }) => (
            <>
              ...
              {fields2.map(({ key, name }) => (
                <Row key={key} gutter={[10, 10]} className="mb-1" wrap={false}>
                  <Col flex={1}>
                    <Form.Item noStyle>
                      <Row gutter={[10, 10]}>
                        <Col span={24}>
