...
export default function PriceTab({ priceCurrency }) {
  ...
  const [specialPricePeriod, setSpecialPricePeriod] = useState(false);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [taxes, setTaxes] = useState([]);
  const [quantityPricePeriod, setQuantityPricePeriod] = useState(false);
        ...
        <Form.List name="specialPrice">
          {(fields, { add, remove }) => (
            <>
              ...
              {fields.map(({ key, name }) => (
                <Row className="mb-1" key={key} wrap={false}>
                  <Col flex={1}>
                    <Form.Item noStyle>
                      <Row gutter={[10, 10]}>
                        <Col span={24}>
        ...
        <Form.List name="quantityPrice">
          {(fields, { add, remove }) => (
            <>
              ...
              {fields.map(({ key, name }) => (
                <Row className="mb-1" key={key} wrap={false} gutter={[10, 10]}>
                  <Col flex={1}>
                    <Form.Item noStyle>
                      <Row gutter={[10, 10]}>
                        <Col span={24}>
