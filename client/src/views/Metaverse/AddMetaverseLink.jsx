import { Button, Checkbox, Form, Image, Input, Row, Switch } from "antd";
...
export default function AddMetaverseLink({
  drowerClose,
  setVerseModalOpen,
  data,
  setDimensions,
  ...
  useEffect(() => {
    if (data) {
      (async () => {
        ...
        form.setFieldValue("isOpenExternalPage", data.isOpenExternalPage);
        form.setFieldValue("metaverseExternalLink", data.metaverseExternalLink);
  ...
  const onFinish = async (values) => {
    try {
      ...
      formData.append("metaverseExternalLink", values.metaverseExternalLink);
      formData.append("isOpenExternalPage", values.isOpenExternalPage);
  ...
  return (
    <Form requiredMark="optional" form={form} onFinish={onFinish} layout="vertical">
      ...
      <Form.Item
        ...
        rules={[{ required: true, message: "Please Enter Dimension Link URL" }]}>
        <Input placeholder="Enter Here" />
      </Form.Item>

      <Form.Item
        name="isOpenExternalPage"
        initialValue={true}
        valuePropName="checked"
      >
        <Checkbox>Open In External Page</Checkbox> 
      </Form.Item>
