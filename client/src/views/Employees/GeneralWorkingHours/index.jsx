...
import { Button, Checkbox, Col, Form, Input, Row, TimePicker, Typography } from "antd";
...
export default function GeneralWorkingHours({ form, employeeId, setGeneralWorkingModal, loading }) {
    ...
    if (employeeId) {
      (async () => {
        try {
          ...
          const res = await EmployeeService.getGeneralWorkingHours(employeeId);
          if (res.data.data.generalWorkingHours) {
            if (Object.keys(res.data.data.generalWorkingHours).length === 0) {
              setIsEdit(false);
            } else {
              setIsEdit(true);
              const days = [];
              for (const [key, value] of Object.entries(res.data.data.generalWorkingHours)) {
    ...
    <>
      <Row style={{ marginBottom: "2rem" }}>
        <Typography.Title level={5}>
          {isEdit ? "Edit" : "Add"} General Working Hours
        </Typography.Title>
      </Row>
      {fetchloading ? (
      ...
      <Row justify="end" style={{ marginTop: "2rem" }}>
        <Col>
          <Row align="middle" gutter={[16, 16]}>
            <Col>
              <Button type="ghost" onClick={() => setGeneralWorkingModal(false)}>
                Cancel
              </Button>
            </Col>
            <Col>
