...
const PassengersDataForm = ({ adults, childs, bookingKey, questions }) => {
  ...
  const personalIdNumber = getQuestionByKey(
    SYSTEM_CONTACT_INFORMTION_KEY.PERSONAL_ID_NUMBER,
    questions,
  );
          ...
          {[...new Array(childs || 0)]?.map((_, index) => {
            return (
              <Row gutter={[12, 20]} key={index}>
                <Form.Item hidden initialValue={"child"} name={["paxes", adults + index, "type"]} />
                <Col span={24}>
