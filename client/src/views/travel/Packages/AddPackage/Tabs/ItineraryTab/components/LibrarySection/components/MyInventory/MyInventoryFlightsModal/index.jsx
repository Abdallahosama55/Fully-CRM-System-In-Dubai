...
import useGetMyInventoryFlights from "services/travel/packages/myInventory/Queries/useGetMyInventoryFlights";
import useAddFlightToLibrary from "services/travel/packages/myInventory/Mutations/useAddFlightToLibrary";
...
const MyInventoryFlightsModal = ({ onCancel, onUpdate, tripId }) => {
  const [form] = useForm();
  ...
  const myInventoryFlights = useGetMyInventoryFlights(
    {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && myInventoryFlights.hasNextPage) {
          myInventoryFlights.fetchNextPage();
        }
      },
      { threshold: 1.0 },
    );
  ...
  }, [myInventoryFlights.hasNextPage]);
      ...
      {activeStep === 1 ? (
        ...
        <>
          ...
          <div className="search_engine_block">
            <Form form={form} onFinish={myInventoryFlights.refetch}>
              ...
              <Row gutter={[12, 12]}>
                ...
                <Col span={3}>
                  <Button
                    ...
                    type="primary"
                    htmlType="submit"
                    loading={myInventoryFlights?.isFetching}
                    className="w-100">
                    Search
                  </Button>
                </Col>
          ...
          <div className="search_engine_results">
            ...
            {myInventoryFlights?.data &&
              myInventoryFlights?.data?.pages
                ?.map((item) => item?.rows)
                .flat()
                ?.map((flight, index) => {
                  return (
...
export default MyInventoryFlightsModal;
