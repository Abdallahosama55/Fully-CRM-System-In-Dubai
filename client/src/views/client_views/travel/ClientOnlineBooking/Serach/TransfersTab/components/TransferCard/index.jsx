...
const TransferCard = ({ transfer, searchInfo }) => {
  ...
  const handleBookClick = () => {
    navigate(`${CLIENT_ROUTER_URLS.BOOKING.BOOK_TRANSFER}${transfer?.id}`, {
      state: {
        ...searchInfo,
    ...
    <div className='transfer_serach_card'>
      ...
      <div>
        ...
        <Typography.Paragraph className="xs_text_regular transfer_serach_card_info_with_icon" level={5} style={{ color: "var(--gray-600)" }}>
          {transfer?.vehicleType?.icon ? <SvgImage svgContent={transfer?.vehicleType?.icon} className="transfer_serach_card_info_with_icon_svg_container" /> : <TransferSVG width={20} height={20} viewBox="0 0 24 24" />}
