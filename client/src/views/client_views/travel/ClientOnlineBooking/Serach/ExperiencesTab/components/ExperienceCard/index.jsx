...
import { GoogleMapsSVG, RateStarSVG, TicketSVG2 } from 'assets/jsx-svg';
    ...
    return (
        <div className='experience_card_search w-100'>
            <MapDrawer
                ...
                center={[Number(experience?.lat), Number(experience?.lng)]}
                markerLocation={[Number(experience?.lat), Number(experience?.lng)]}
            />
            ...
            <Row gutter={16}>
                        ...
                        <div>
                            <div className='experience_card_head_line space-between'>
                                ...
                                <div className="experience_card_theme_imgs">
                                    {experience.productThemes.map(el => <Tooltip key={el.id} title={el.name}>
                                        <SvgImage
                                            className="experience_card_theme_img"
                                            svgContent={el.image}
                                        />
                                    </Tooltip>)}
                                </div>
                            ...
                            <Typography.Paragraph
                                ...
                                style={{ color: "#313342" }}
                            >{experience?.location}</Typography.Paragraph>
                        </div>
                        <div className="google_map_link_box items-center">
                            <a className="google_map_link fz-14 fw-600"
                                onClick={() => setOpenMap(true)}
                            ><GoogleMapsSVG /> View on map</a>
                        </div>
                        <div>
                            <Typography.Paragraph
                                ellipsis={{
                ...
                <Col xl={4} lg={4} md={6} xs={24}>
                    <div className="price_section">
                        <p className="fz-18 fw-700" style={{ marginBottom: "0" }}>${experience.finalPrice}</p>
                        <p className="fz-14 fw-500">{experience.priceType === PASSENGER_PRICE_TYPE.PER_PERSON ? "Per person" : "Per booking"}</p>
                        {experience.freeSeats && <p className="fz-14 fw-500 rooms_left">{experience.freeSeats} Seats Left</p>}
                        <Link to={CLIENT_ROUTER_URLS.BOOKING.VIEW_EXPERIANCE + experience?.bookingKey + "?" + query}>
                            <Button size="small" type="primary" style={{ marginTop: experience.freeSeats ? "" : "8px", padding: "10px 30px" }}>Book</Button>
                        </Link>
                    </div>
                </Col>
            </Row>
