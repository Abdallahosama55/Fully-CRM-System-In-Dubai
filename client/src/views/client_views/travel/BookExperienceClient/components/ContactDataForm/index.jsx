import { Col, DatePicker, Form, Input, Row, Select, Skeleton } from 'antd'
...
const ContactDataForm = ({ questions }) => {
    ...
    const gender = useMemo(() => getQuestionByKey(SYSTEM_CONTACT_INFORMTION_KEY.GENDER, questions), [questions]);
    const address = useMemo(() => getQuestionByKey(SYSTEM_CONTACT_INFORMTION_KEY.ADDRESS, questions), [questions]);
    const dateOfBirth = useMemo(() => getQuestionByKey(SYSTEM_CONTACT_INFORMTION_KEY.DATE_OF_BIRTH, questions), [questions]);
    // const email = useMemo(() => getQuestionByKey(SYSTEM_CONTACT_INFORMTION_KEY.EMAIL, questions), [questions]);
    const language = useMemo(() => getQuestionByKey(SYSTEM_CONTACT_INFORMTION_KEY.LANGUAGE, questions), [questions]);
    const nationality = useMemo(() => getQuestionByKey(SYSTEM_CONTACT_INFORMTION_KEY.NATIONALITY, questions), [questions]);
    const organization = useMemo(() => getQuestionByKey(SYSTEM_CONTACT_INFORMTION_KEY.ORGANIZATION, questions), [questions]);
    const passportExpiry = useMemo(() => getQuestionByKey(SYSTEM_CONTACT_INFORMTION_KEY.PASSPORT_EXPIRY, questions), [questions]);
    const passportId = useMemo(() => getQuestionByKey(SYSTEM_CONTACT_INFORMTION_KEY.PASSPORT_ID, questions), [questions]);
    const personalIdNumber = useMemo(() => getQuestionByKey(SYSTEM_CONTACT_INFORMTION_KEY.PERSONAL_ID_NUMBER, questions), [questions]);
    // const phoneNumber = useMemo(() => getQuestionByKey(SYSTEM_CONTACT_INFORMTION_KEY.PHONE_NUMBER, questions), [questions]);
    const title = useMemo(() => getQuestionByKey(SYSTEM_CONTACT_INFORMTION_KEY.TITLE, questions), [questions]);
    ...
    return (
        <Row gutter={[12, 12]}>
            ...
            <Col {...getColProps(questions.length)}>
                <Form.Item
